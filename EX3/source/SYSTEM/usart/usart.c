 ...
 * usart.c 是本工程的“调参接口”。
 *
 * 本实验没有使用蓝牙 APP 协议，而是设计了更适合课堂演示的 ASCII 命令：
...
static void ascii_print_status(void)
{
     ...
     */
    sprintf(buf,
            "STATUS,STOP=%d,MODE=%d,BK=%d,BD=%d,VK=%d,VI=%d,MID=%d,ANGLE=%d,GYRO=%d,EL=%d,ER=%d,ML=%d,MR=%d\r\n",
            Flag_Stop, (int)Way_Angle, (int)Balance_Kp, (int)Balance_Kd,
            (int)Velocity_Kp, (int)Velocity_Ki, Middle_angle,
            (int)Angle_Balance, (int)Gyro_Balance, Encoder_Left, Encoder_Right,
...
static void ascii_cmd_handle(char *cmd)
{
    ...
    {
        ascii_puts("CMD: BAL kp kd | VEL kp ki | PID bk bd vk vi | MID angle | MODE DMP/KALMAN/COMP | ARM | STOP | STATUS\r\n");
    }
    else
    {
