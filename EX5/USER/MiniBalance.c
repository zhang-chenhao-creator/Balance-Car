...
/*
 * EX4：B585 平衡小车蓝牙 + OLED + 级联 PID 集成版
 *
 * 说明：
 * 1. 本工程入口文件不是 main.c，而是 Keil 工程中的 USER/MiniBalance.c。
 *    下面的 int main(void) 就是程序主入口。
 * 2. 本版本面向“平衡站立 + 蓝牙遥控 + OLED 状态显示”验收，核心闭环链路为：
 *    MPU6050 姿态采集 -> 编码器速度反馈 -> 角度环 PD -> 速度环 PI -> 电机 PWM。
 * 3. USART1 保留 ASCII 调试命令，便于查询 STATUS 和临时调整 PID；
 *    USART3 接蓝牙模块，OLED_STATUS 负责屏幕状态页。
 * 4. 控制公式在 MiniBalance/CONTROL/control.c 中实现，DMP 初始化和读取在
 *    MiniBalance_HARDWARE/MPU6050/MPU6050.c 中封装。
 */
...
/* USART3 蓝牙中断与控制中断共享的运动状态。 */
volatile u16 Flag_front = 0, Flag_back = 0, Flag_Left = 0, Flag_Right = 0;
volatile u16 Flag_velocity = 2, Target_Velocity = 300;
volatile u8 Bluetooth_Command = BT_CMD_STOP;
float RC_Velocity = 0, RC_Turn_Velocity = 0;

/*
 * 软件停机标志。上电默认允许平衡，是否输出 PWM 仍由 KEY2 决定。
 * USART1 发送 STOP 后会锁停，发送 ARM 才能重新允许输出。
 */
volatile u8 Flag_Stop = 0;
volatile u8 Safety_Stop_Reason = 0;
u16 determine = 0;
...
u32 Distance = 0;
volatile u8 Ultrasonic_Valid = 0, Ultrasonic_Obstacle = 0;
volatile u8 Ultrasonic_Avoid_Enable = 1, Ultrasonic_Avoid_Action = ULTRA_STATE_CLEAR;
volatile u8 Ultrasonic_Miss_Count = 0;
u8 delay_65 = 0, PID_Send = 0;
...
float Turn_Kp = 4200, Turn_Kd = 100;
...
int main(void)
{
    ...
    while (1)
    {
        ...
        delay_flag = 1;
        while (delay_flag);
        Ultrasonic_Service_65ms();
        OLED_Status_Update_65ms();
    }
}
