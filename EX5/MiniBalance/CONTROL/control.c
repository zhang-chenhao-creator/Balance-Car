...
volatile int Ultrasonic_Request_Speed = 0, Ultrasonic_Safe_Speed = 0;
volatile u16 Ultrasonic_Stop_Distance = OBSTACLE_GUARD_STOP_BASE_MM;
volatile u16 Ultrasonic_Slow_Distance =
    OBSTACLE_GUARD_STOP_BASE_MM + OBSTACLE_GUARD_SLOW_MARGIN_MM;

static ObstacleGuardContext ultrasonic_guard;
static u8 ultrasonic_guard_applied_enable;

static float Forward_Movement_Target(float speed)
{
    return speed / Perimeter / Control_Frequency *
           EncoderMultiples * Reduction_Ratio * Encoder_precision * 2;
}

void Ultrasonic_Guard_Init(void)
{
    ObstacleGuard_Init(&ultrasonic_guard, Ultrasonic_Avoid_Enable);
    ultrasonic_guard_applied_enable = Ultrasonic_Avoid_Enable;
    Ultrasonic_Avoid_Action = Ultrasonic_Avoid_Enable ?
                              ULTRA_STATE_CLEAR : ULTRA_STATE_DISABLED;
}

void Ultrasonic_Guard_SetEnable(u8 enable)
{
    /* USART handlers only publish the requested setting. The control ISR owns
       and updates the guard context, so it cannot be changed mid-update. */
    Ultrasonic_Avoid_Enable = enable ? 1U : 0U;
}
...
int Velocity(int encoder_left, int encoder_right)
{
    ...
    ObstacleGuardInput guard_input;
    ObstacleGuardOutput guard_output;
    int requested_speed = 0;
    int measured_forward_speed;

    /*
     * 速度环 PI：负责让小车不要一直往前/往后跑。
     *
     * encoder_left + encoder_right 表示车整体前后运动趋势。
     * 蓝牙未发前进/后退命令时，目标速度为 0，所以误差写成 0 - 当前速度。
     * 蓝牙发前进/后退命令时，Movement 会把目标速度换算成编码器计数目标。
     */
    Encoder_Least = 0 - (encoder_left + encoder_right);

    if (Flag_front == 1)
        requested_speed = Target_Velocity;
    else if (Flag_back == 1)
        requested_speed = -(int)Target_Velocity;

    measured_forward_speed = (int)((Velocity_Left + Velocity_Right) / 2.0f);
    if (measured_forward_speed < 0)
        measured_forward_speed = 0;

    Ultrasonic_GetSnapshot(&ultrasonic);
    if (ultrasonic_guard_applied_enable != Ultrasonic_Avoid_Enable)
    {
        ObstacleGuard_SetEnabled(&ultrasonic_guard, Ultrasonic_Avoid_Enable);
        ultrasonic_guard_applied_enable = Ultrasonic_Avoid_Enable;
    }
    guard_input.requested_speed_mm_s = requested_speed;
    guard_input.measured_forward_speed_mm_s = measured_forward_speed;
    guard_input.distance_mm = ultrasonic.distance_mm;
    guard_input.sample_id = ultrasonic.sample_id;
    guard_input.sample_valid = ultrasonic.valid;
    guard_input.miss_count = ultrasonic.miss_count;
    guard_output = ObstacleGuard_Update(&ultrasonic_guard, &guard_input);

    Ultrasonic_Request_Speed = requested_speed;
    Ultrasonic_Safe_Speed = (int)guard_output.allowed_speed_mm_s;
    Ultrasonic_Stop_Distance = guard_output.stop_mm;
    Ultrasonic_Slow_Distance = guard_output.slow_mm;
    Ultrasonic_Avoid_Action = guard_output.state;
    Ultrasonic_Obstacle = (guard_output.state == OBSTACLE_GUARD_BLOCKED) ? 1U : 0U;

    Movement = Forward_Movement_Target((float)guard_output.allowed_speed_mm_s);
    if (Movement > 2400) Movement = 2400;
    if (Movement < -2400) Movement = -2400;
     ...
     */
    if (guard_output.entered_blocked)
    {
        /* Drop old forward integral and use current speed immediately to brake. */
        Encoder_Integral = 0;
...
int Turn(float gyro)
{
    ...
    float Kp = Turn_Kp;
    float Kd = 0;
    float divider = (Flag_velocity == 0) ? 1.0f : (float)Flag_velocity;

    if (Flag_Left == 1)
        Turn_Target = -54.0f / divider;
    ...
    return (int)(Turn_Target * Kp / 100.0f + gyro * Kd / 100.0f);
}
...
u8 Turn_Off(float angle, int voltage)
{
    ...
    Safety_Stop_Reason = SAFE_RUN;
    if (angle < -40 || angle > 40)
        Safety_Stop_Reason = SAFE_FALL;
    else if (KEY2_STATE == 1)
        Safety_Stop_Reason = SAFE_KEY;
    else if (Flag_Stop == 1)
        Safety_Stop_Reason = SAFE_SOFT_STOP;
    else if (voltage < 1000)
        Safety_Stop_Reason = SAFE_LOW_VOLT;
    else if (Pick_up_stop == 1)
        Safety_Stop_Reason = SAFE_PICK_UP;

    if (Safety_Stop_Reason != SAFE_RUN)
    {
        temp = 1;
