...
{ 		
	...
	if(GPIO_Pin==GPIO_PIN_9)
	{
		...
		ELE_Mode();                                         //���ѭ��ģʽ
		CCD_Mode();                                         //CCDѲ��ģʽ����
		if(Mode==Track_Line_Patrol_Mode)  IRDM_line_inspection();
		if(Mode==Track_Line_Patrol_Mode)  TrackAvoid_Supervisor5ms();
...
{  
    ...
    if(Mode==Track_Line_Patrol_Mode)
			Movement=Patrol_Speed_Cmd/Perimeter/Control_Frequency*EncoderMultiples*Reduction_Ratio*Encoder_precision*2;
...
int Turn(float gyro)
{
	...
	//===================ң��������ת����=================//
	 ...
	 else if(Mode==Track_Line_Patrol_Mode)  Turn_Target=Patrol_Turn_Cmd;
		 else Turn_Target=0;
