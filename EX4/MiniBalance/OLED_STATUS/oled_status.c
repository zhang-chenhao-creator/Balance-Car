...
#define OLED_REFRESH_TICKS 4
#define OLED_PAGE_TICKS    60
...
static void OLED_Draw_Remote_Page(void)
{
    ...
    OLED_ShowString(0, 48, (const u8 *)"TGT:");
    OLED_ShowNumber(32, 48, Target_Velocity, 3, 12);
    OLED_ShowString(56, 48, (const u8 *)"mm/s");
}
...
void OLED_Status_Update_50ms(void)
{
    refresh_ticks++;
    page_ticks++;
